...
func _ready() -> void:
	max_value = AppSettings.get_setting("multiply_max_value")
...
func generate_next_question() -> Array[int]:
	...
	while _previous_key == key or regenerate:
		_question_a = randi_range(1, max_value)
		_question_b = randi_range(1, max_value)
		
		key = str(_question_a, "x", _question_b)
...
func _should_regenerate(key: String) -> bool:
	var current = int(MultiplyMemory.get_setting(key))
...
func _correct_answer() -> void:
	var key = str(_question_a, "x", _question_b)
	var current = MultiplyMemory.get_setting(key)
	current += 1
...
func generate_correctness_map(total: int) -> Dictionary:
	...
	for i in range(total):
		...
		var key = str(questions[0], "x", questions[1])
